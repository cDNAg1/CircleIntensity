...
# ===================
#
# 2017/03/03 - Implemented analysis of dPCR experiments
# 2017/06/08 - Measuring fluorescence of circles instead of squares
# 2017/06/23 - Fitting Grid
# 2017/06/29 - Saving panel number of circles as well
# 2017/07/15 - Fix intensities + new function for intesity callculation
# 2017/07/26 - Callculation of concentrations
# 2017/07/26 - Imporved saving of data
# 2017/08/04 - Added analysis differentiation between beads (dark positive) and flourescent labeled cells (bright positive)
#
###########################################################################################
            ...
            for y in range((int(centery-radius-2)),int(round(centery+radius+2))):       # Varying through y
                 if x <= image.shape[1]-1 and y <= image.shape[0]-1 and x>=0 and y>=0:  # Making sure coordinate in image
                     ...
                     if pixeldistance <= radius:                                        # If Pixel is in circle add to intensity callculation
                         brightness = brightness + float(image[y,x])/255                # Updating total brightness
...
for x in range (0,NPan):
    ...
    for i in circles[0,:]:
        intensity = CircleIntensity(i[0],i[1],i[2]-1,gimg)
        # Wirting to file
